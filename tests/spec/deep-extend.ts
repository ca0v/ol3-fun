...
describe("utils/deep-extend", () => {
  it("trivial merges", () => {
    ...
    shouldEqual(
      ...
      stringify([]),
      "empty arrays with empty object"
    );
    shouldEqual(
      stringify(deepExtend([,], [, ,])),
      stringify([,]),
    ...
    should(
      o !== deepExtend(o),
      "clones when second argument not provided"
    );
  });
  ...
  it("invalid merges", () => {
    shouldThrow(
      () => deepExtend(<any>{}, []),
      "array->object considered an error"
    );
    shouldThrow(
      () =>
        deepExtend(<any>{ a: 1 }, []),
  ...
  it("merges with duplicate objects that might be detected as recursive", () => {
    ...
    let p = { o1: o, o2: o };
    shouldEqual(
      stringify(deepExtend(p)),
      stringify(p),
      "two children pointing to the same object"
    );
    let q = {
      p1: p,
      p2: [p],
  ...
  it("simple array merges", () => {
    shouldEqual(
      stringify(deepExtend([1], [])),
      stringify([1]),
      "[1] + []"
    );
    shouldEqual(
      stringify(deepExtend([1], [2])),
      stringify([2]),
      "[1<-2]"
    );
    shouldEqual(
    ...
    shouldEqual(
      ...
      stringify([1, 2, 3, 4]),
      "array can have empty items"
    );
    should(
      deepEqual(
        deepExtend([1, 2, 3], <any>{
          1: 100,
        }),
        [1, 100, 3]
      ),
  ...
  it("clones nested objects", () => {
    ...
    should(
      source.v1.v2 !== o.v1.v2,
      "properly clones v1.v2"
    );
  });
  ...
  it("confirms array merge is 'id' aware", () => {
    ...
    shouldEqual(
      o.values[0].value.v1,
      1,
      "object is clone of o1, v1"
    );
    shouldEqual(
      o.values[1].value.v2,
      1,
      "object is clone of o1, v2"
    );
    shouldEqual(
      o.values[2].value.v9,
      1,
      "object is clone of o1, v9"
    );
  ...
  it("confirms trace on simple array merging", () => {
    ...
    shouldEqual(
      trace[0].key,
      1,
      "key: target[*1*] was 2 and is now 3"
    );
    shouldEqual(
      trace[0].was,
      2,
      "was: target[1] was *2* and is now 3"
    );
    shouldEqual(
      trace[0].value,
      3,
      "value: target[1] was 2 and is now *3*"
    );
  });
  ...
  it("confirms trace diff on simple array", () => {
    ...
    shouldEqual(
      stringify(diff(trace)),
      stringify({ 1: 3 }),
      "simple array trace diff"
    );
  });
  ...
  it("confirms trace content", () => {
    ...
    let target = deepExtend(
      ...
      },
      {
        foo: 1,
        property:
          "should fire 'add' event with this object and string path to it",
        object: {
          p1: "p1",
          p2: 2,
          a1: [1, 2, 3],
    ...
    shouldEqual(
      ...
      "no trivial trace elements"
    );
    shouldEqual(
      trace.map((t) => t.key).join(" "),
      "property object",
      "changes are depth first"
    );
