...
import { Snapshot } from "../../ol3-fun/snapshot";
...
describe("Snapshot", () => {
  ...
  it("Converts a polygon to image data", () => {
    ...
    let originalCoordinates = stringify(
      geom.getCoordinates()
    );
    let data = Snapshot.snapshot(
      feature,
    ...
    show(data);
    let finalCoordinates = stringify(
      geom.getCoordinates()
    );
    shouldEqual(
      originalCoordinates,
    ...
    if (1 === window.devicePixelRatio) {
      ...
      shouldEqual(
        data,
        circleData,
        "circle data as expected"
      );
    }
  });
});

function createStyle(text = ""): Style {
  return new Style({
    fill: new Fill({
      color: "black",
    }),
