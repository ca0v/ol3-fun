...
describe("doif tests", () => {
    let die = (n: any) => { throw `doif callback not expected to execute: ${n}`; };
    let spawn = () => {
        let v = true;
        return () => v = !v;
    };
...
describe("mixin tests", () => {
    ...
    it("nested mixins", () => {
        shouldEqual(mixin({ vermont: { burlington: true } }, { ["south carolina"]: { greenville: true } })["south carolina"].greenville, true, "greenville is in south carolina");
        shouldEqual(mixin({ vermont: { burlington: true } }, { vermont: { greenville: false } }).vermont.greenville, false, "greenville is not in vermont");
        shouldEqual(mixin({ vermont: { burlington: true } }, { vermont: { greenville: false } }).vermont.burlington, undefined, "second vermont completely wipes out 1st");
    });
});
