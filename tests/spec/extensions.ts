...
describe("data/extensions", () => {
  ...
  it("ensures two objects can be bound to same extension data", () => {
    let x = new Extensions();
    let math = x.extend(Math, {
      sqrt2: Math.sqrt(2),
    });
    should(
      !!(<any>x.extend(Math)).sqrt2,
      "Math.sqrt2"
    );
    x.bind(Number, Math);
    shouldEqual(
  ...
  it("ensures two extensions can bind data to the same object", () => {
    ...
    ext2.extend(o, { ext: 2 });
    shouldEqual(
      (<any>ext1.extend(o)).ext,
      1,
      "ext1"
    );
    shouldEqual(
      (<any>ext2.extend(o)).ext,
      2,
      "ext2"
    );
  });
  ...
  it("extension references are preserved", () => {
    ...
    shouldEqual(
      p.name,
      "P",
      "extension references are preserved"
    );
  });
  ...
  it("extension integrity testing (100 objects X 10 extensions)", () => {
    ...
    let data = range(1000).map((n) =>
      Object.create({ id: n })
    );
    data.map((d, i) =>
      x[i % 10].extend(d, {
        data: shuffle(range(1000)),
      })
    );
    data.forEach((d, i) => {
      let data = (<
  ...
  it("extensions performance testing (1 million accesses)", () => {
    ...
    data.forEach(
      (d) =>
        ...
        >x.extend(d)).counter.count++
    );
    shouldEqual(
      counter.count,
      data.length,
      `accessed ${data.length} items`
    );
  }).timeout(1000);
});
